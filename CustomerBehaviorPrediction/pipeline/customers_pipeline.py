...
@dsl.pipeline(name='Customer Pipeline', description='Training models for Customer Expenses prediction.')
def customers_pipeline(train_set: str, retrain: bool):
    ...
    # Run load_data task
    load_task = load(train_set, 'testset.csv', retrain)

    # Run preprocess_data task
    preprocess_task = preprocess(load_task.outputs['TrainRawData'], load_task.outputs['TestRawData'])

    # Run tasks "linear_regression", "xgboost_regressor" and "neural_regression" given
    # the output generated by "load_task".
    linear_regression_task = linear_regression(preprocess_task.outputs['Data'], retrain)
    xgboost_regressor_task = xgboost_regressor(preprocess_task.outputs['Data'], retrain)
    neural_regression_task = neural_regression(preprocess_task.outputs['Data'], retrain)

    # Given the outputs from "linear_regression", "xgboost_regressor" and "neural_regression" task
    # the component "show_results" is called to print the results.
    show_results(linear_regression_task.outputs['Mae_Train'], linear_regression_task.outputs['Mae_Test'],
                 linear_regression_task.outputs['Params'],
                 xgboost_regressor_task.outputs['Mae_Train'], xgboost_regressor_task.outputs['Mae_Test'],
                 xgboost_regressor_task.outputs['Params'],
                 neural_regression_task.outputs['Mae_Train'], neural_regression_task.outputs['Mae_Test'],
                 neural_regression_task.outputs['Params'])

    # Given the outputs from "linear_regression", "xgboost_regressor" and "neural_regression" task
    # the component "evaluate_best_model" is called to print the best models.
